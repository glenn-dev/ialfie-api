...
const createUser = (req, res) => {
  const { first_n, last_n, email, password, phone, id_number, buildings, departments } = req.body
  pool.query(
    'INSERT INTO users (first_n, last_n, email, password, phone, id_number) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id', 
    [first_n, last_n, email, password, phone, id_number],
    (error, results) => {
    if (error) {
      throw error;
    };
...
const updateUser = (req, res) => {
  const { id, first_n, last_n, email, password, phone, id_number, buildings, departments } = req.body;
  pool.query(
    'UPDATE users SET first_n = $1, last_n = $2, email = $3, password = $4, phone = $5, id_number = $6 WHERE id = $7',
    [first_n, last_n, email, password, phone, id_number, id],
    (error, results) => {
      if (error) {
        throw error;
      };
